...
typedef Vector4DF	VectorBuffer;
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow )
{
	...
	// Init window
    if( FAILED( InitWindow( hInstance ) ) )  return 0;

	// Init device
	if( FAILED( InitDevice() ) )  { CleanupDevice(); return 0; }
    
	// User init
	char* title = initialize ( lpCmdLine );
...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
	...
	const wchar_t *str2 = L"StateViewer (c) NVIDIA, by R.Hoetzlein - DIRECTX 11";
